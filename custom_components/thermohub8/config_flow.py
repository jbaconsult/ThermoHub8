...
async def _validate_connection(hass: HomeAssistant, data: Dict[str, Any]) -> Dict[str, Any]:
    """Prüft Verbindung und Beispiel-Payload."""
    session = async_get_clientsession(hass)
    client = ThermoHub8Client(
        session=session,
        base_url=data[CONF_BASE_URL], # Zugriff über data[] weil Pflichtfeld
        api_key=data.get(CONF_API_KEY),
        verify_ssl=data.get(CONF_VERIFY_SSL, True),
    )
    _LOGGER.info("Validating ThermoHub8 connection to %s", data.get(CONF_BASE_URL))
    ...
    async def async_step_user(self, user_input: Dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                ...
                # Basis-URL als unique_id verwenden
                await self.async_set_unique_id(user_input[CONF_BASE_URL].rstrip("/"))
                self._abort_if_unique_id_configured()
                _LOGGER.info("ThermoHub8 creating config entry for %s", user_input.get(CONF_BASE_URL))
                return self.async_create_entry(title="ThermoHub8", data=user_input)

        return self.async_show_form(step_id="user", data_schema=STEP_USER_DATA_SCHEMA, errors=errors)

    async def async_step_import(self, user_input: Dict[str, Any]) -> FlowResult:
        """Unterstützung für YAML-Import."""
        return await self.async_step_user(user_input)

    async def async_step_reauth(self, user_input: Dict[str, Any] | None = None) -> FlowResult:
        """Re-Auth falls API-Key/Basis-URL nicht mehr gültig."""
        return await self.async_step_user(user_input)
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    """Optionen für bestehende Konfiguration (z. B. Abfrageintervall)."""
    ...
    async def async_step_init(self, user_input: Dict[str, Any] | None = None) -> FlowResult:
        if user_input is not None:
            _LOGGER.info("ThermoHub8 options updated: %s", user_input)
